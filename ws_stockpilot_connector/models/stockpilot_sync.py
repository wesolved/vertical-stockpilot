...
class StockpilotSync(models.Model):
    ...
    def _get_stockpilot_orders(self, config, last_sync_date=None):
        ...
        try:
            ...
            params = {
                'page': 1,
                'page_size': 50,
                'modified_since': last_sync_date.isoformat() if last_sync_date else None
            }
        ...
        for config in configs:
            try:
                ...
                for order_data in order_list:
                    try:
                        if self._process_stockpilot_order(order_data, config.company_id):
                            success_count += 1
                    except Exception as e:
                        _logger.error(f"Failed to process order {order_data.get('order_number')}: {str(e)}")

                _logger.info(f"Successfully imported {success_count}/{len(order_list)} orders")
    ...
    def _process_stockpilot_order(self, order_data, company):
        ...
        try:
            ...
            if not partner:
                _logger.error(f"Failed to find/create customer for order {order_data.get('order_number')}")
                return False
            ...
            if order_data.get('order_placed_dt'):
                try:
                    order_date = datetime.strptime(
                        order_data.get('order_placed_dt'),
                        '%Y-%m-%dT%H:%M:%S%z'
                    )
                    # Convert to naive datetime
                    order_date = order_date.replace(tzinfo=None)
                except ValueError as e:
                    _logger.warning(f"Invalid date format, using current date: {str(e)}")
            ...
            order_vals = {
                ...
                'order_line': self._prepare_order_lines(order_data.get('order_details', {}).get('line_items', [])),
            ...
            shipping_total = float(order_data.get('order_details', {}).get('shipping_total', 0))
            if shipping_total > 0:
            ...
            _logger.error(f"Failed to process order {order_data.get('order_number')}: {str(e)}", exc_info=True)
            return False
        ...
        for line in line_items:
            try:
                ...
                product = self.env['product.product'].search([
                    ('name', '=ilike', line['sales_channel_title'].split('-')[0].strip())
                ], limit=1)

                if not product:
                    _logger.warning(f"Product not found for: {line['sales_channel_title']}")
                    product = self._create_placeholder_product(line['sales_channel_title'])

                price = float(line.get('retail_price', 0)) or float(line.get('total_price', 0))
                if price <= 0:
