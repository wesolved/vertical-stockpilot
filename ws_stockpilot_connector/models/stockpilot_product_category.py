...
class StockPilotProductCategory(models.Model):
    _name = "stockpilot.product.category"
    ...
    def _create_stockpilot_category(self):
        """
        Execute an API call to Stockpilot to create a category for this record.
        ...
        response = connection._execute_post_request("categories/create", product_data)
        self.stockpilot_id = response.get("id")

    def create(self, vals):
        """
        Override create to also create the category in Stockpilot
        ...
        if self.env.context.get("skip_delay"):
            res._create_stockpilot_category()
        else:
            res.with_delay()._create_stockpilot_category()
        return res
